...
function ReportView({ report, profile, onModify, onShare }: { report: CareerReport; profile: Profile; onModify: () => void; onShare: () => void }) {
  ...
  return <section className="report-shell">
    ...
    <JobVerificationPanel key={report.requestId} report={report} profile={profile} />
    <section className="sources-section"><div className="section-title"><span>职业判断依据</span><h2>资料有出处，岗位有时效</h2><p className="section-description">这里的资料用于解释职业内容与进入门槛，不等同于正在招聘的具体岗位；可投递状态只以上方动态核验区为准。</p></div><div className="source-list">{report.sources.map((source) => <article id={`source-${source.id}`} key={source.id}><div><EvidenceBadge label={source.label} /><span className={`confidence confidence-${source.confidence}`}>{source.confidence}</span></div><a href={source.url} target="_blank" rel="noopener noreferrer">{source.title} ↗</a><p>{source.publisher} · {source.region}</p><small>{source.supports}</small><footer>资料时间：{source.publishedOrCheckedAt} · 获取：{source.retrievedAt.slice(0,10)}</footer></article>)}</div></section>
