  ...
  return (
    <header className="site-header">
      <button className="brand brand-button" type="button" onClick={onHome} aria-label="返回职途罗盘首页">
        <span className="brand-mark" aria-hidden="true">北</span><span>职途罗盘</span>
      </button>
      {stage === "intro" ? <span className="demo-pill">SSAI 黑客松 Demo</span> : (
...
function ProfileForm({ profile, setProfile, onBack, onSubmit, isLoading, error }: { profile: Profile; setProfile: (profile: Profile) => void; onBack: () => void; onSubmit: () => void; isLoading: boolean; error: string }) {
  ...
  return <section className="flow-shell profile-layout">
    ...
    <form className="profile-form panel" onSubmit={(event) => { event.preventDefault(); onSubmit(); }}>
      ...
      <div className="field compact-grid"><div><label htmlFor="weeklyTime"><span>6</span> 每周可投入</label><select id="weeklyTime" value={profile.weeklyTime} onChange={(event) => update("weeklyTime", event.target.value)}><option>3 小时以内</option><option>5—8 小时</option><option>8—12 小时</option><option>12 小时以上</option></select></div><div><label htmlFor="budget">学习预算</label><select id="budget" value={profile.budget} onChange={(event) => update("budget", event.target.value)}><option>0 元</option><option>500 元以内</option><option>500—2000 元</option><option>2000 元以上</option></select></div><div><label htmlFor="location">地区 / 远程要求</label><input id="location" value={profile.location} onChange={(event) => update("location", event.target.value)} maxLength={100} /></div></div>
      <div className="field"><label><span>7</span> 你最看重哪些工作特征？ <b>*</b></label><ChipSelector options={VALUE_OPTIONS} selected={profile.workValues} onChange={(values) => update("workValues", values.slice(0, 4))} label="看重的工作特征，最多四项" /></div>
...
function QuestionFlow({ questions, answers, setAnswers, onBack, isFallback }: { questions: Question[]; answers: Answer[]; setAnswers: (answers: Answer[]) => void; onBack: () => void; isFallback: boolean }) {
  ...
  return <section className="flow-shell questions-layout">
    ...
    <div className="question-panel panel">
      <div className="question-top"><span className={`mentor-avatar mentor-tone-${question.mentor}`}>{MENTOR_META[question.mentor].icon}</span><div><small>{MENTOR_META[question.mentor].name}</small><strong>{index + 1} / {questions.length}</strong></div></div>
...
function ReportView({ report, onModify, onShare }: { report: CareerReport; onModify: () => void; onShare: () => void }) {
  ...
  return <section className="report-shell">
    ...
    <div className={`data-notice source-${report.sourceMode}`}><span>{report.sourceMode === "live" ? "● 实时资料" : report.sourceMode === "mixed" ? "◐ 部分检索" : "◇ 缓存降级"}</span><p>{report.dataNotice}</p></div>
    <section className="observation-section"><div className="section-title"><span>导师观察</span><h2>导师团如何理解你</h2></div><div className="observation-grid">{report.mentorObservations.map((item) => <article key={item.mentor} className={`observation-card mentor-tone-${item.mentor}`}><span className="mentor-avatar">{MENTOR_META[item.mentor].icon}</span><small>{MENTOR_META[item.mentor].name}</small><p>{item.observation}</p><details><summary>查看对应回答</summary><ul>{item.supportingAnswers.map((answer) => <li key={answer}>{answer}</li>)}</ul></details></article>)}</div></section>
...
export default function CareerExplorer() {
  ...
  return <main>
    ...
    <footer className="site-footer"><span>职途罗盘 · SSAI Hackathon Demo</span><p>职业探索不是职业定论。请用真实任务和可信来源继续验证。</p></footer>
  </main>;
}
