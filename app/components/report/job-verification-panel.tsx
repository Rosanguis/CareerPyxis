...
import type { ApiErrorBody, CareerReport, JobOpportunityLead, JobPathInput, JobSearchProfile, JobVerificationResponse, Profile } from "@/lib/types";
...
export function JobVerificationPanel({ report, profile, onLeadsChange }: { report: CareerReport; profile: Profile; onLeadsChange: (leads: JobOpportunityLead[]) => void }) {
  ...
  return <section className="job-verification-section" aria-labelledby="job-verification-title">
    ...
    <div className="verification-paths">{report.rankedPaths.map((path, index) => {
      ...
      const label = state?.status === "verifying" ? "核验中" : state?.status === "queued" ? "等待核验" : state?.status === "error" ? "未完成" : result?.status === "verified" ? `找到 ${result.jobs.length} 个` : result?.leads.length ? `${result.leads.length} 条官网线索` : result?.status === "mock" ? "演示模式" : "暂无合格岗位";
