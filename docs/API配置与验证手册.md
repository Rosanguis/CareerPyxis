...
报告生成成功后，前端会对三个职业方向依次调用 `POST /api/job-verification`。该接口沿用同一个 provider 的搜索能力，再通过 Greenhouse、Lever 或 Ashby 官方公开 API 二次确认岗位是否仍发布；它不会延长主报告等待时间。
...
- 只有受支持 ATS 的官方 API 仍返回岗位、官方申请入口存在、且当前已知地区/方向/阶段无明确冲突时，页面才显示“已核验可投”。未知条件必须作为“投递前仍需确认”展示。
