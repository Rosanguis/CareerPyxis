...
DeepSeek 原生搜索参数对齐 `deepseek-ai/deepseek-harness`：单次搜索请求最多使用 5 次 `web_search_20250305`，`max_tokens=4096`。应用层每条查询最多接收 6 个结果、每个方向最多读取 8 个官方候选；其中北森详情候选最多保留 4 条，并可从最多 2 个安全北森企业租户的公开职位列表补充最多 4 条结构化候选。扩大的是官方候选发现范围，不会绕过状态、地区、阶段和匹配核验。国内校招会额外执行北森来源查询，因此总查询层数为四层。
