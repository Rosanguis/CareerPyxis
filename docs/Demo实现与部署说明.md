...
3. 只接受 Greenhouse、Lever、Ashby、SmartRecruiters 四类固定 HTTPS 岗位详情地址，避免请求任意外部地址。
...
- 岗位核验目前覆盖 Greenhouse、Lever、Ashby、SmartRecruiters；同义岗位名和相邻起步岗会显式标注扩展原因，不会伪装成报告中的精确职业。“核验时仍在招”只表示核验时官方接口仍公开，且与用户已经填写的条件未发现明确冲突，不保证录取，也无法替代签证、薪资和工作许可确认。
