...
2. 只接受 Greenhouse、Lever、Ashby 三类固定 HTTPS 岗位详情地址，避免请求任意外部地址。
3. 调用对应官方公开 API 二次确认该岗位当前仍发布、申请入口仍存在。
4. 模型只对已由官方 API 确认的候选做方向、地区、远程范围和职业阶段匹配；外部岗位文本按不可信数据处理。
5. 未通过任何一层的候选直接排除。薪资、签证、工作许可和具体用工条件缺失时必须提示用户投递前确认。
...
- 岗位核验目前只覆盖 Greenhouse、Lever、Ashby；“已核验可投”只表示核验时官方接口仍公开，且与用户已经填写的条件未发现明确冲突，不保证录取，也无法替代签证、薪资和工作许可确认。
