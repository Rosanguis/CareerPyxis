...
- Vercel 函数部署区域为香港，最长执行 150 秒；应用服务端硬截止为 120 秒，浏览器请求上限为 125 秒，并保留更短的分阶段超时。这样调整是因为 2026-08-22 的 Live Preview 联网报告实测达到 75.7 秒，需要为 DeepSeek/Harness 抖动增加余量；平台上限比应用截止多 30 秒，可避免 Vercel 先强制终止并返回不可控 504，让应用尽量返回可理解的结构化超时错误。
