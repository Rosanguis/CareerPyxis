...
3. 只接受 Greenhouse、Lever、Ashby、SmartRecruiters、Workday 五类固定 HTTPS 岗位详情地址，避免请求任意外部地址；Workday 还会校验 `posted=true` 与 `canApply=true`。
...
- 岗位核验目前覆盖 Greenhouse、Lever、Ashby、SmartRecruiters、Workday；报告方向会始终合并常见英文市场职位名，避免模型只生成中文检索词。检索使用三条较短的分层查询，并在画像明确为应届/学生时加入阶段词。同义岗位名和相邻起步岗会显式标注扩展原因，不会伪装成报告中的精确职业。“核验时仍在招”只表示核验时官方接口仍公开且可申请，并与用户已经填写的条件未发现明确冲突，不保证录取，也无法替代签证、薪资和工作许可确认。
